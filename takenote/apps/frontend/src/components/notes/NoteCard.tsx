...
export default function NoteCard({ note }: NoteCardProps) {
  const { handleOpenEdit, handleOpenDelete } = useNotesContext();
  const colorKey = (note.color in ZEN_COLORS) ? note.color : "slate";
  const styles = ZEN_COLORS[colorKey];
  const Icon = CATEGORY_ICON_MAP[note.category as keyof typeof CATEGORY_ICON_MAP] || FolderOpen;
  ...
  return (
    ...
    >
      ...
      <div className="px-5.5 py-5 flex flex-col gap-2.5">
        ...
        <p className="text-zinc-650 dark:text-zinc-350 text-xs leading-relaxed font-semibold line-clamp-5 whitespace-pre-line">
          {note.content}
        </p>
      </div>
